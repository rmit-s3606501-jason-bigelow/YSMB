...
public class MonteCarloGuessPlayer extends RandomGuessPlayer implements Player{
    ...
    public void updateKillerOdds(MonteCoord givenLock)
    {
        ...
        for(ships = 0; ships < targetsLeft.length; ships++)
        {
            ...
            while( i < targetsLeft[ships] -1 && stillClear && givenLock.col - i > 0)
            {
            ...
            i = 0;
            stillClear = true;
            while( i < targetsLeft[ships] -1 && stillClear && givenLock.col + i < this.numCol - 1)
            {
                if(seekGrid[givenLock.col + i][givenLock.row].state == 'h' || seekGrid[givenLock.col + i][givenLock.row].state == 'n')
                {
                    i++;
            ...
            i = 0;
            stillClear = true;
            while( i < targetsLeft[ships] -1 && stillClear && givenLock.row + i < this.numRow - 1)
            {
            ...
            i = 0;
            stillClear = true;
            while( i < targetsLeft[ships] -1 && stillClear && givenLock.row - i > 0)
            {
                if(seekGrid[givenLock.col][givenLock.row - i].state == 'h' || seekGrid[givenLock.col][givenLock.row - i].state == 'n')
                {
                    i++;
            ...
            if(spacesAbove + spacesBelow >= targetsLeft[ships])
            {
                ...
                for(i = spacesBelow; i > 0; i--)
                {
                    ...
                    {
                        seekGrid[givenLock.col][givenLock.row - i].killerOdds += Math.min(maxVal, spacesBelow - i);
                    }
                }
                for(i = spacesAbove; i > 0; i--)
                {
                    if(seekGrid[givenLock.col][givenLock.row + i].state == 'n')
                    {
                        seekGrid[givenLock.col][givenLock.row + i].killerOdds += Math.min(maxVal, spacesAbove - i);
                    }
                }
            }
            if(spacesLeft + spacesRight >= targetsLeft[ships])
            {
                ...
                for(i = spacesLeft; i > 0; i--)
                {
                    ...
                    {
                        seekGrid[givenLock.col - i][givenLock.row].killerOdds += Math.min(maxVal, spacesLeft - i);
                    }
                }
                for(i = spacesRight; i > 0; i--)
                {
                    if(seekGrid[givenLock.col + i][givenLock.row].state == 'n')
                    {
                        seekGrid[givenLock.col + i][givenLock.row].killerOdds += Math.min(maxVal, spacesRight - i);
                    }
                }
            }
        }
